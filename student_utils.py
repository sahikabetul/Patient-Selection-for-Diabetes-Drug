...
def reduce_dimension_ndc(df, ndc_df):
    ...
    '''
    # we only need the names, not the whole table
    ndc_df = ndc_df[['NDC_Code', 'Non-proprietary Name']] 
    # changing the columns names for easier merge
    ndc_df.columns = ['ndc_code', 'generic_drug_name'] 
    
    temp_df = df.copy()
    # merge two dataframes on "ndc_code"
    temp_df = pd.merge(temp_df, ndc_df, on='ndc_code')# temp_df.merge(ndc_df, on="ndc_code", how="left")
...
def select_first_encounter(df):
    ...
    '''
    # sort df values by 'encounter_id', drop duplicated 'patient_nbr' as keeping first of them.
...
def patient_dataset_splitter(df, patient_key='patient_nbr'):
    ...
    '''
    # take 'patient_key' column values, randomly permute them
    keys = df[patient_key].values
    np.random.seed(seed= 42)
    keys = np.random.permutation(keys)
    
    # define sizes of train, val parts by length of keys
    size = len(keys)
    train_size = int(0.6*size) # 60% train
    validation_size = int(0.2*size) # 20% valid and 20% test

    # split data to train, val, test by defined sizes
    ...
    print("Train set unique patient number : ", len(train['patient_nbr'].unique()))
    print("Valid set unique patient number : ", len(validation['patient_nbr'].unique()))
    print("Test  set unique patient number : ", len(test['patient_nbr'].unique()))
